document.addEventListener("DOMContentLoaded", function () {
  ...
  function createArticleCardHTML(item) {
    ...
    return '<article class="article-card">' +
      ...
      categoryLabel + "</span>" +
      '<img src="/img/' + item.slug +
      '.png" onerror="this.onerror=null;this.src=\'/img/default-' +
      item.category + '.png\'" alt="' + item.title +
  ...
  function createSnippetCardHTML(item) {
    return '<article class="snippet-card" data-snippet-content="' +
      ...
      '<h3 class="snippet-card__title">' + item.title +
      ' <span class="snippet-card__copy-icon" aria-hidden="true">⧉</span></h3>' +
      '<p class="snippet-card__description">' + item.description + "</p>" +
      '<pre class="snippet-card__content"><code>' +
      item.content.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(
        />/g,
        "&gt;",
      ) + "</code></pre>" +
      "</div>" +
  ...
  function createLinkCardHTML(item) {
    return '<article class="link-card">' +
      '<a href="' + item.url +
      '" class="link-card__link" target="_blank" rel="noopener noreferrer">' +
      '<span class="link-card__category">#' + item.category + "</span>" +
      '<div class="link-card__body">' +
      '<h3 class="link-card__title">' + item.title +
      ' <span class="link-card__external-icon" aria-hidden="true">↗</span></h3>' +
  ...
  function performSearch(query) {
    ...
    if (articleResults.length === 0) {
      grid.innerHTML =
        '<p class="search-results__empty">該当する記事が見つかりませんでした</p>';
    } else {
    ...
    // スニペット結果を表示
    const snippetsSection = searchResultsContainer.querySelector(
      ".snippets-section--search",
    );
    const snippetsGrid = searchResultsContainer.querySelector(
      ".snippets-section__grid--search",
    );
    ...
    // リンク結果を表示
    const linksSection = searchResultsContainer.querySelector(
      ".links-section--search",
    );
    const linksGrid = searchResultsContainer.querySelector(
      ".links-section__grid--search",
    );
